/**
 * Only one section can be open at a time — opening one closes any other
  ...
  return (
    <ul className="space-y-3">
      {services.map((service, idx) => {
        ...
        return (
          <li key={service.key} className={idx !== services.length - 1 ? "border-b border-white/20 pb-3" : ""}>
            <button
              type="button"
            ...
            {/* CSS-grid height trick: animates from 0fr to 1fr so the accordion
                can smoothly expand/collapse without knowing the content height ahead of time. */}
            <div
